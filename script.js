...
function onPointerDown(e) {
    ...
    activePointers[e.pointerId] = e.target
    if (e.clientX < canvas.width / 2) {
        handleLeftSwipeStart(e);
    }else{
        handleDashPointer(e);
    }
}
function onPointerUp(e) {
    const target = activePointers[e.pointerId]
    if (target === meowBtn) {
    ...
    if (e.clientX < canvas.width / 2) {
        handleLeftSwipeEnd(e);
    }else{
        handleDashPointerEnd(e);
    }
}
const buttons = [meowBtn]
buttons.forEach(btn => {
...
function handleDashPointer(e) {
  ...
  dashPointerData.active = true;
  dashPointerData.startX = e.clientX;
  dashPointerData.startY = e.clientY;
  dashPointerData.currentX = e.clientX;
  dashPointerData.currentY = e.clientY;
  document.addEventListener('pointermove', handleDashPointerMove);
  document.addEventListener('pointerup', handleDashPointerEnd);
}

function handleDashPointerMove(e) {
    if (!dashPointerData.active) return;
    dashPointerData.currentX = e.clientX;
    dashPointerData.currentY = e.clientY;
}

function handleDashPointerEnd(e) {
    if (!dashPointerData.active) return;
    dashPointerData.active = false;
    const dx = dashPointerData.currentX - dashPointerData.startX;
    const dy = dashPointerData.currentY - dashPointerData.startY;
    const dist = Math.sqrt(dx * dx + dy * dy);
    if (dist >= 20) {
        const angle = Math.atan2(dy, dx);
        performSwipeDash(angle);
    }
    document.removeEventListener('pointermove', handleDashPointerMove);
    document.removeEventListener('pointerup', handleDashPointerEnd);
}
...
function handleLeftSwipeStart(e) {
    leftSwipeData.active = true;
    leftSwipeData.startX = e.clientX;
    leftSwipeData.startY = e.clientY;
    leftSwipeData.currentX = e.clientX;
    leftSwipeData.currentY = e.clientY;
    document.addEventListener('pointermove',handleLeftSwipeMove)
    document.addEventListener('pointerup', handleLeftSwipeEnd);

}
function handleLeftSwipeMove(e){
    if(!leftSwipeData.active) return;
    leftSwipeData.currentX=e.clientX
    leftSwipeData.currentY=e.clientY
}
function handleLeftSwipeEnd(e) {
    if (!leftSwipeData.active) return;
    leftSwipeData.active = false;
    const dx = leftSwipeData.currentX - leftSwipeData.startX;
    const dy = leftSwipeData.currentY - leftSwipeData.startY;
    const dist = Math.sqrt(dx * dx + dy * dy);
    if (dist >= 20) {
        const angle = Math.atan2(dy, dx);
        ...
        leftSwipeData.lastTap=Date.now()
        leftSwipeData.tapCount++

    }else{
        const currentTime=Date.now()
        if(currentTime-leftSwipeData.lastTap<300){
            leftSwipeData.tapCount++
            if(leftSwipeData.tapCount>=2){
                handleJump()
                leftSwipeData.tapCount=0
            }
        }else{
            leftSwipeData.lastTap=currentTime
            leftSwipeData.tapCount=1
        }
    }
   document.removeEventListener('pointermove',handleLeftSwipeMove)
   document.removeEventListener('pointerup', handleLeftSwipeEnd);
}
